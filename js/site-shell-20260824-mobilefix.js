...
${items}
      ...
      <div class="nav-brand-wrap">
        <a class="nav-brand" href="${siteHref('index.html')}" aria-label="WestTech Home Automation home">
          <img
            ...
            src="${siteHref('images/WestTech_Logo_BuiltSmart_MadeCustom_20260824.png')}"
            width="2048"
            height="655"
            alt="WestTech Home Automation – Built Smart, Made Custom"
          />
          <span class="nav-logo-fallback" hidden aria-hidden="true">
            <strong>WestTech</strong>
            <span>Home Automation</span>
            <small>BUILT SMART • MADE CUSTOM</small>
          </span>
        </a>
      </div>
  ...
  if (headerHost) {
    ...
    // Hard mobile-safe fallback: if the logo asset ever fails to decode/load,
    // remove the broken-image box instead of letting it inflate the fixed header.
    const headerLogo = headerHost.querySelector('.nav-logo-full');
    const headerLogoFallback = headerHost.querySelector('.nav-logo-fallback');
    if (headerLogo && headerLogoFallback) {
      headerLogo.addEventListener('error', () => {
        ...
        headerLogoFallback.setAttribute('aria-hidden', 'false');
        requestAnimationFrame(syncFixedHeaderOffset);
      }, { once: true });
    }
