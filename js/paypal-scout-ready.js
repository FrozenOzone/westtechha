(function () {
  ...
  async function init() {
    try {
      ...
      const config = await fetchJson(CONFIG_ENDPOINT, {
        method: "GET",
        headers: {
      ...
      await window.paypal.Buttons({
        ...
        onError(err) {
          console.error(err);
          setStatus("PayPal checkout hit an error. Check your PayPal settings, Cloudflare variables, and Functions deployment.", true);
        }
      }).render("#paypal-button-container");

      setStatus("Secure PayPal checkout is ready.");
    } catch (error) {
      console.error(error);
      setStatus(error.message || "PayPal checkout could not be initialized.", true);
    }
  }
