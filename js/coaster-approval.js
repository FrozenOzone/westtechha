(function(){
  ...
  function render(){
    ...
    $('#cap-set').textContent=`${Number(approval.setSize||4)}-Coaster Set`;$('#cap-set-count').textContent=String(approval.setCount||1);$('#cap-total-coasters').textContent=String(approval.totalCoasters||approval.setSize||4);$('#cap-fulfillment').textContent=fulfillment(approval.fulfillmentMethod);$('#cap-production-window').textContent=approval.productionWindow||'Confirmed before production';
    const designWork=money(approval.artworkCharge)+money(approval.billableWorkTotal);
    const payStatus=String(approval.paymentStatus||'').toUpperCase(),paid=payStatus==='PAID';const knownTax=taxReview?money(taxReview.taxAmount):money(approval.taxAmount);const knownPaymentTotal=taxReview?money(taxReview.totalAmount):(money(approval.paymentTotal)||money(approval.finalAmount));const showPaymentTotal=!!taxReview||paid;const taxResolved=!!taxReview||paid;const taxPending=approval.paymentRequired&&!taxResolved;
    $('#cap-base-total').textContent=fmt(money(approval.basePrice)*Math.max(1,Number(approval.setCount||1)));$('#cap-art-charge').textContent=fmt(designWork);$('#cap-other-charge').textContent=fmt(approval.otherCharge);$('#cap-shipping').textContent=fmt(approval.shippingAmount);$('#cap-discount').textContent=`−${fmt(approval.discountAmount)}`;const taxRow=$('#cap-tax-row'),taxLabel=$('#cap-tax-label'),taxAmount=$('#cap-tax-amount');if(taxPending){taxLabel.textContent=approval.fulfillmentMethod==='SHIP'?'Colorado sales tax (if applicable)':'Colorado sales tax';taxAmount.textContent=approval.fulfillmentMethod==='SHIP'?'Calculated after PayPal address':'Calculated before payment';taxRow.hidden=false;taxRow.classList.add('pending');}else{taxLabel.textContent='Colorado sales tax';taxAmount.textContent=fmt(knownTax);taxRow.hidden=knownTax===0&&!paid;taxRow.classList.remove('pending');}$('#cap-final-total').textContent=fmt(showPaymentTotal?knownPaymentTotal:approval.finalAmount);$('#cap-total-label').textContent=showPaymentTotal?'PAYMENT TOTAL':'APPROVED SUBTOTAL';
    ...
    if(approval.paymentRequired){
      $('#cap-payment-note').textContent=shipped?'The approved subtotal above does not yet include sales tax. If the shipping address you confirm in PayPal is in Colorado, applicable address-based sales tax will be calculated, added, and shown for confirmation before payment is captured. Out-of-state shipments are not charged Colorado sales tax.':'Colorado sales tax is required for local pickup. It will be calculated using WestTech’s pickup address, added to the approved subtotal, and shown for confirmation before PayPal captures payment.';
