...
import { sendCoasterAdminNewOrderEmail } from '../../../_lib/coaster-admin-email.js';
export async function onRequestPost(context){
  try{
    ...
    const adminEmail=await sendCoasterAdminNewOrderEmail(context.env,{order,requestUrl:context.request.url});
    return jsonResponse({ok:true,orderId:order.orderId,status:order.status,email,adminEmail},201);
