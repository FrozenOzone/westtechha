...
export async function onRequestPost(context){
  try{const body=await context.request.json();const approvalToken=String(body?.approvalToken||'');const paypalOrderId=String(body?.paypalOrderId||'');if(!approvalToken||!paypalOrderId)return jsonResponse({ok:false,message:'Missing PayPal return details.'},400);const order=await captureCoasterPayPalOrder(context.env,context.params.orderId,approvalToken,paypalOrderId);if(order&&String(order.status||'').toUpperCase()==='IN_PRODUCTION')await sendCoasterAdminProductionEmail(context.env,{order,requestUrl:context.request.url});const approval=await getCoasterApprovalByToken(context.env,context.params.orderId,approvalToken);return jsonResponse({ok:true,approval});}
