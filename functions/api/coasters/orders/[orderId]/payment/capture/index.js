...
export async function onRequestPost(context){
  try{
    ...
    if(result?.capturedNow&&result.order){if(String(result.order.status||'').toUpperCase()==='PRODUCTION_QUEUE')await sendCoasterAdminProductionEmail(context.env,{order:result.order,requestUrl:context.request.url});await inviteCustomerForOrder(context.env,{sourceType:'COASTER',order:result.order,requestUrl:context.request.url});}
