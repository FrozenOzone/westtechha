...
export async function onRequestPost(context){try{requireCoasterAdmin(context);const before=await getCoasterOrderDetail(context.env,context.params.orderId);if(!before)return jsonResponse({ok:false,message:'Coaster order not found.'},404);const body=await context.request.json();if(String(body?.action||'')==='previewMarkPaid'){const order=await markPreviewPaid(context,before);return jsonResponse({ok:true,order,previewPaymentSimulated:true});}const order=await updateCoasterOrderAdmin(context.env,context.params.orderId,body);if(before.status!==order.status&&['IN_PRODUCTION','PREPARING_TO_SHIP','PREPARING_FOR_PICKUP','READY_FOR_PICKUP','SHIPPED','COMPLETED'].includes(order.status))await sendCoasterCustomerEmail(context.env,{type:order.status,order,requestUrl:context.request.url});return jsonResponse({ok:true,order});}catch(error){return jsonResponse({ok:false,message:error.message||'Could not update coaster order.'},error.status||500);}}
