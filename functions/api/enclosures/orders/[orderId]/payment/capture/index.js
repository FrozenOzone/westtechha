...
export async function onRequestPost(context){
  try{
    ...
    const result=await captureEnclosurePayPalOrder(context.env,context.params.orderId,approvalToken,paypalOrderId,{confirmTax:body?.confirmTax===true});if(result?.capturedNow&&result.order){await sendEnclosureAdminProductionEmail(context.env,{order:result.order,requestUrl:context.request.url});await inviteCustomerForOrder(context.env,{sourceType:'ENCLOSURE',order:result.order,requestUrl:context.request.url});}
