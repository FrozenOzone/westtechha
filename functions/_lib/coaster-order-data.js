...
function mapRow(r){if(!r)return null;return {
  ...
  customerName:r.customer_name,customerEmail:r.customer_email,customerPhone:r.customer_phone||'',mobileCarrier:r.mobile_carrier||'OTHER',communicationPreference:r.communication_preference||'EMAIL',smsConsent:!!Number(r.sms_consent),
  ...
  estimatedPrinterMinutes:Number(r.estimated_printer_minutes||0),printerAssignment:r.printer_assignment||'',productionWindow:r.production_window||'',
...
export async function createCoasterOrder(env,form){
  ...
  const customerName=clean(form.get('customerName'),120),customerEmail=clean(form.get('customerEmail'),254).toLowerCase(),customerPhone=clean(form.get('customerPhone'),60),mobileCarrier=normalizeMobileCarrier(form.get('mobileCarrier')),communicationPreference=clean(form.get('communicationPreference'),20).toUpperCase(),smsConsent=bool(form.get('smsConsent'));if(customerName.length<2)throw makeError('Enter your name.');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail))throw makeError('Enter a valid email address.');if(customerPhone.replace(/\D/g,'').length<10)throw makeError('Enter a valid mobile phone number.');if(!mobileCarrier)throw makeError('Choose your mobile carrier.');if(!['EMAIL','SMS'].includes(communicationPreference))throw makeError('Choose Email or Text message as your preferred communication method.');if(communicationPreference==='SMS'&&!smsConsent)throw makeError('Consent to receive text-message notifications is required when Text message is selected.');if(communicationPreference==='SMS'&&!carrierSupportsGateway(mobileCarrier))throw makeError('Email-to-text is not available for that carrier. Choose Email as your preferred communication method.');
