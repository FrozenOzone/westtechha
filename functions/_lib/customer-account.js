...
export async function reorderCustomerOrder(env,account,sourceType,sourceOrderId,requestUrl=''){
  ...
  if(type==='COASTER'){
    ...
    let selectedComponents=[];try{selectedComponents=JSON.parse(source.loaded_components_json||'[]').filter(component=>component?.selected&&!component?.required).map(component=>component.componentSku);}catch{}const configured=buildConfiguredProduct(source.sku,source.quantity,selectedComponents),quantity=Math.max(1,Number(source.quantity||1)),startingUnitPrice=positiveAmount(source.starting_unit_price,configured.unitAmount),startingSubtotal=positiveAmount(source.starting_subtotal,startingUnitPrice*quantity),productAmount=positiveAmount(source.product_amount,startingSubtotal),fulfillment=FULFILLMENT.has(clean(source.fulfillment_method,30).toUpperCase())?clean(source.fulfillment_method,30).toUpperCase():'UNSET',shippingFallback=fulfillment==='SHIP'?shippingAmountForQuantity(quantity):0,shippingAmount=fulfillment==='LOCAL_PICKUP'?0:positiveAmount(source.shipping_amount,shippingFallback),customCharge=amount(source.custom_charge),discountAmount=amount(source.discount_amount),calculated=Math.max(0,productAmount+customCharge+shippingAmount-discountAmount),finalAmount=positiveAmount(source.final_amount,calculated);
    await db.prepare(`INSERT INTO enclosure_orders (
      order_id,order_date,daily_sequence,status,
      customer_name,customer_email,customer_phone,communication_preference,sms_consent,
