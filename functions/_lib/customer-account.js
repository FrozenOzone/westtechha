...
async function sendReorderNotifications(env,{account,sourceType,sourceOrderId,newOrderId,requestUrl}){
  ...
  ]),sms=await sendTransactionalSms(env,{sourceType:type,order:{orderId:newOrderId,customerPhone:account.phone,communicationPreference:account.communicationPreference,smsConsent:account.smsConsent},eventType:'CUSTOMER_REORDER_RECEIVED',message:transactionalSmsText({subject:'We received your WestTech reorder request',orderId:newOrderId,url:portalUrl}),idempotencyKey:`sms-customer-reorder-${newOrderId}`});return {customer,admin,sms};
}
