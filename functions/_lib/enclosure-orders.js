...
function mapRow(row){
  ...
  return {
    ...
    customerName:row.customer_name,customerEmail:row.customer_email,customerPhone:row.customer_phone||'',communicationPreference:row.communication_preference||'EMAIL',smsConsent:!!Number(row.sms_consent),
...
export async function createEnclosureOrder(env,body){
  ...
  const customerName=clean(body?.customerName,120),customerEmail=clean(body?.customerEmail,254).toLowerCase(),customerPhone=clean(body?.customerPhone,60),communicationPreference=clean(body?.communicationPreference,20).toUpperCase(),smsConsent=bool(body?.smsConsent);if(customerName.length<2)throw makeError('Enter your name.');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail))throw makeError('Enter a valid email address.');if(customerPhone.replace(/\D/g,'').length<10)throw makeError('Enter a valid mobile phone number.');if(!['EMAIL','SMS'].includes(communicationPreference))throw makeError('Choose Email or Text message as your preferred communication method.');if(communicationPreference==='SMS'&&!smsConsent)throw makeError('Consent to receive text messages is required when Text message is selected.');if(body?.requestConfirmed!==true)throw makeError('Confirm that this is a review request and that payment comes later.');
  ...
  await db.prepare(`INSERT INTO enclosure_orders (order_id,order_date,daily_sequence,status,customer_name,customer_email,customer_phone,communication_preference,sms_consent,sku,family,model,board_variant,offer_type,color,quantity,starting_unit_price,starting_subtotal,loaded_components_json,loaded_components_amount,customer_notes,fulfillment_preference,product_amount) VALUES (?,?,?,'REQUEST_RECEIVED',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(orderId,orderDate,dailySequence,customerName,customerEmail,customerPhone,communicationPreference,smsConsent?1:0,sku,clean(product.family,80),model,clean(product.variant,80).includes('38')?'38':'30',clean(product.offerType,40),color,quantity,unitPrice,startingSubtotal,jsonDetail(loadedComponents),loadedComponentsAmount,clean(body?.customerNotes,2500),preference,startingSubtotal).run();
