...
import { buildConfiguredProduct, getProduct } from './product.js';
...
const DISPLAY_COMPONENT_SKUS=new Set(['oled-096','lcd2004']);
...
function parseComponentJson(value){try{const parsed=JSON.parse(String(value||'[]'));return Array.isArray(parsed)?parsed:[];}catch(e){return [];}}
...
function mapRow(row){
  ...
  return {
    ...
    customerName:row.customer_name,customerEmail:row.customer_email,customerPhone:row.customer_phone||'',mobileCarrier:row.mobile_carrier||'OTHER',communicationPreference:row.communication_preference||'EMAIL',smsConsent:!!Number(row.sms_consent),
    sku:row.sku,family:row.family,model:row.model,boardVariant:row.board_variant,offerType:row.offer_type,color:row.color,quantity:Number(row.quantity||1),startingUnitPrice:number(row.starting_unit_price),startingSubtotal:number(row.starting_subtotal),loadedComponents:parseComponentJson(row.loaded_components_json),loadedComponentsAmount:number(row.loaded_components_amount),customerNotes:row.customer_notes||'',
...
export async function createEnclosureOrder(env,body){
  ...
  const customerName=clean(body?.customerName,120),customerEmail=clean(body?.customerEmail,254).toLowerCase(),customerPhone=clean(body?.customerPhone,60),mobileCarrier=normalizeMobileCarrier(body?.mobileCarrier),communicationPreference=clean(body?.communicationPreference,20).toUpperCase(),smsConsent=bool(body?.smsConsent);if(customerName.length<2)throw makeError('Enter your name.');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail))throw makeError('Enter a valid email address.');if(customerPhone.replace(/\D/g,'').length<10)throw makeError('Enter a valid mobile phone number.');if(!mobileCarrier)throw makeError('Choose your mobile carrier.');if(!['EMAIL','SMS'].includes(communicationPreference))throw makeError('Choose Email or Text message as your preferred communication method.');if(communicationPreference==='SMS'&&!smsConsent)throw makeError('Consent to receive text-message notifications is required when Text message is selected.');if(communicationPreference==='SMS'&&!carrierSupportsGateway(mobileCarrier))throw makeError('Email-to-text is not available for that carrier. Choose Email as your preferred communication method.');if(body?.requestConfirmed!==true)throw makeError('Confirm that this is a review request and that payment comes later.');
  const sku=clean(body?.sku,100).toLowerCase(),baseProduct=getProduct(sku);if(!baseProduct)throw makeError('Choose a valid WestTech enclosure configuration.');
  ...
  const selectedComponentSkus=Array.isArray(body?.loadedComponentSkus)?body.loadedComponentSkus.map(value=>clean(value,80).toLowerCase()).filter(Boolean):[];
  const dualDisplaySelected=[...DISPLAY_COMPONENT_SKUS].every(componentSku=>selectedComponentSkus.includes(componentSku));
  if(dualDisplaySelected&&body?.displayCombinationAcknowledged!==true)throw makeError('Confirm that both the OLED and LCD2004 displays are intentionally selected.');
  let product;try{product=buildConfiguredProduct(sku,quantity,selectedComponentSkus);}catch(error){throw makeError(error.message||'Choose valid enclosure components.',error.status||400);}
  const unitPrice=money(product.unitAmount),startingSubtotal=money(product.itemAmount),loadedComponents=(product.loadedComponents||[]).map(component=>dualDisplaySelected&&DISPLAY_COMPONENT_SKUS.has(component.componentSku)?{...component,dualDisplayAcknowledged:true}:component),loadedComponentsAmount=money(product.loadedComponentsAmount),{orderId,orderDate,dailySequence}=await allocateOrderId(db),model=clean(body?.modelLabel,100)||clean(product.variant,100)||clean(product.family,80);
  await db.prepare(`INSERT INTO enclosure_orders (order_id,order_date,daily_sequence,status,customer_name,customer_email,customer_phone,mobile_carrier,communication_preference,sms_consent,sku,family,model,board_variant,offer_type,color,quantity,starting_unit_price,starting_subtotal,loaded_components_json,loaded_components_amount,customer_notes,fulfillment_preference,product_amount) VALUES (?,?,?,'REQUEST_RECEIVED',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(orderId,orderDate,dailySequence,customerName,customerEmail,customerPhone,mobileCarrier,communicationPreference,smsConsent?1:0,sku,clean(product.family,80),model,clean(product.variant,80).includes('38')?'38':'30',clean(product.offerType,40),color,quantity,unitPrice,startingSubtotal,jsonDetail(loadedComponents),loadedComponentsAmount,clean(body?.customerNotes,2500),preference,startingSubtotal).run();
  await logEnclosureEvent(db,orderId,'REQUEST_SUBMITTED',{status:'REQUEST_RECEIVED',sku,quantity,color,fulfillmentPreference:preference,startingSubtotal,loadedComponents:loadedComponents.filter(component=>component.selected).map(component=>component.label),dualDisplayAcknowledged:dualDisplaySelected});return getEnclosureOrderDetail(env,orderId);
}
...
export async function releaseEnclosureTerms(env,orderId,{refreshOnly=false}={}){
  ...
  if(!['SHIP','LOCAL_PICKUP'].includes(order.fulfillmentMethod))throw makeError('Choose Ship Order or Local Pickup before sending the configuration.',409);if(Number(order.estimatedPrinterMinutes||0)<=0)throw makeError('Enter the estimated printer minutes before sending the configuration.',409);if(!clean(order.productionWindow,160))throw makeError('Enter the estimated production window before sending the configuration.',409);if(order.paymentRequired&&money(order.finalAmount)<=0)throw makeError('Set the reviewed subtotal, or turn Payment Required off, before sending the configuration.',409);
