...
export async function sendCoasterAdminProductionEmail(env,{order,requestUrl=''}={}){
  ...
  const key=`coaster-admin-production-${clean(order.orderId,80).replace(/[^A-Za-z0-9_.:-]/g,'-')}`.slice(0,240);
  if(await hasSent(env,order.orderId,key))return {sent:true,duplicate:true,idempotencyKey:key};
  if(!apiKey){await logEvent(env,order.orderId,'EMAIL_NOT_CONFIGURED',{emailType:'ADMIN_PRODUCTION_RELEASE',to,idempotencyKey:key});return {sent:false,skipped:true,reason:'RESEND_API_KEY not configured',idempotencyKey:key};}
  ...
  const subject=paid?`PAYMENT RECEIVED — ${order.orderId} — RELEASED TO PRODUCTION`:`CUSTOMER APPROVED — ${order.orderId} — RELEASED TO PRODUCTION`;
  const headline=paid?'Payment received — build can start.':'Customer approved — build can start.';
  ...
  const rows=[
    ...
    ['Production status','IN PRODUCTION'],['Payment',paymentLine],['Fulfillment',fulfillment],
  ...
  try{
    ...
    await logEvent(env,order.orderId,'EMAIL_SENT',{emailType:'ADMIN_PRODUCTION_RELEASE',to,subject,provider:'RESEND',providerId:data?.id||null,idempotencyKey:key});
    return {sent:true,id:data?.id||null,idempotencyKey:key};
  }catch(error){
    await logEvent(env,order.orderId,'EMAIL_FAILED',{emailType:'ADMIN_PRODUCTION_RELEASE',to,subject,message:clean(error?.message,600),idempotencyKey:key});
    return {sent:false,error:clean(error?.message,600)||'Admin production email failed.',idempotencyKey:key};
  }
}
