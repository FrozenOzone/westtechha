...
function template(type,order,{approvalUrl=''}) {
  ...
  const deliveryNote=type==='CUSTOM_ORDER_READY'?'<p style="font-size:13px;line-height:1.55;color:#617187;margin:18px 0 0;">If you do not see future WestTech updates, please check Spam or Junk and add <strong>orders@westtechha.com</strong> to your contacts.</p>':'';
  const html=frame(headline,`${paragraphs}${rows(details)}${button(label,url)}${deliveryNote}`);
  const text=[headline,'',intro,'',process,'',action,'',...details,'',url?`${label}: ${url}`:'',type==='CUSTOM_ORDER_READY'?'If a WestTech update is missing, check Spam or Junk and add orders@westtechha.com to your contacts.':''].filter(Boolean).join('\n');
