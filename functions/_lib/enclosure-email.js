...
function frame(title,content,siteUrl='',customerFacing=false){
  const contact=siteUrl?`<p style="font-size:12px;line-height:1.5;color:#74839a;margin:18px 0 0;">Questions? Reply to this email or contact <a href="mailto:orders@westtechha.com" style="color:#1677C4;">orders@westtechha.com</a>.<br><a href="${esc(siteUrl)}" style="color:#1677C4;">WestTech ESP32 Enclosures</a></p>`:'';
  const inboxNote=customerFacing?`<p style="font-size:12px;line-height:1.5;color:#74839a;margin:12px 0 0;">Missing a WestTech email? Check your Junk/Spam folder or search your mailbox for <strong>orders@westtechha.com</strong>.</p>`:'';
  const footer=`${contact}${inboxNote}`;
...
export async function sendEnclosureRequestEmails(env,{order,requestUrl=''}){
  ...
  const customerKey=`enclosure-customer-request-${order.orderId}`,customer=await send(env,{to:order.customerEmail,subject:customerSubject,html:frame(`Your enclosure request is in, ${order.customerName}.`,customerContent,'',true),text:`Your enclosure request is in.\n\nOrder: ${order.orderId}\nConfiguration: ${item}\n${components.join('\n')}${components.length?'\n':''}Starting product subtotal: ${money(order.startingSubtotal)}\n\nWestTech will review the configuration, final price, workload, and estimated production window before payment. No payment has been collected.\n\nMissing a WestTech email? Check your Junk/Spam folder or search your mailbox for orders@westtechha.com.`,replyToAddress:replyTo(env),idempotencyKey:customerKey,orderId:order.orderId,emailType:'REQUEST_RECEIVED'}),sms=await sendTransactionalSms(env,{sourceType:'ENCLOSURE',order,eventType:'REQUEST_RECEIVED',message:transactionalSmsText({subject:customerSubject,orderId:order.orderId}),idempotencyKey:`sms-${customerKey}`});
  const root=siteBase(env,requestUrl),adminUrl=`${root}/admin/enclosure-orders.html?order=${encodeURIComponent(order.orderId)}`;
  const adminContent=`<p style="font-size:15px;line-height:1.65;color:#42526a;">A new enclosure request was submitted. No payment has been collected.</p>${detailRows([`Order: ${order.orderId}`,`Customer: ${order.customerName} — ${order.customerEmail}`,`Mobile: ${order.customerPhone||'—'} • ${order.mobileCarrier||'OTHER'} • ${order.communicationPreference||'EMAIL'}`,`Configuration: ${item}`,...components,`Customer notes: ${order.customerNotes||'None'}`])}${button('Open Enclosure Request',adminUrl)}`;
  const admin=await send(env,{to:adminTo(env),subject:`NEW ENCLOSURE REQUEST — ${order.orderId} — ${order.customerName}`,html:frame('New enclosure request',adminContent),text:`NEW ENCLOSURE REQUEST\n\nOrder: ${order.orderId}\nCustomer: ${order.customerName} — ${order.customerEmail}\nMobile: ${order.customerPhone||'—'} • ${order.mobileCarrier||'OTHER'} • ${order.communicationPreference||'EMAIL'}\nConfiguration: ${item}\n${components.join('\n')}${components.length?'\n':''}Notes: ${order.customerNotes||'None'}\n\nOpen: ${adminUrl}`,replyToAddress:order.customerEmail,idempotencyKey:`enclosure-admin-request-${order.orderId}`,orderId:order.orderId,emailType:'ADMIN_NEW_ORDER'});
...
function customerTemplate(type,order,{approvalUrl='',siteUrl=''}){
  ...
  const html=frame(headline,`${paragraphs}${detailRows(lines)}${button(label,url)}`,siteUrl,true);
  const text=[headline,'',intro,'',action,'',...lines,'',url?`${label}: ${url}`:'','', 'Missing a WestTech email? Check your Junk/Spam folder or search your mailbox for orders@westtechha.com.'].filter(Boolean).join('\n');return {subject,html,text,smsUrl:url};
}
